...
const GradeAUser = () => {
    ...
    const filteredLoanList = AllLoans?.filter((item) => item.installments.length == 2);
    // const testDate = filteredLoanList?.[0]?.installments?.[filteredLoanList?.[0]?.installments?.length - 1]?.date;
    const currentDate = dateFormat(new Date(), 'yyyy-mm-dd');

    // console.log(moment(currentDate).isSame(testDate, 'month'), 'filteredLoanList');
    // const totalInstallments = filteredLoanList?.[0]?.installmentsDate?.filter((item) => item < currentDate)?.length;
    // const paidInstallments = filteredLoanList?.[0]?.installments?.length;
    const Defaulters = AllLoans?.filter(
        (i) => i?.installmentsDate?.filter((item) => item < currentDate)?.length == i?.installments?.length
    );
    const usersData = Defaulters?.map((item, index) => {
        return {
    ...
    return (
        <Grid container spacing={gridSpacing}>
            <Grid item xs={12}>
                <Card>
                    <h3 className={classes.title}>Grade A Customers</h3>
                    <Formik
                        initialValues={{
                            type: '',
                            date: '',
                                ...
                                <Grid item xs={12} className={classes.flex}>
                                    ...
                                    <Grid item xs={10} className={classes.flex}>
                                        ...
                                        <Grid item xs={3} style={{ marginTop: '10px' }}>
                                            <GradeBCustomers loans={filteredLoanList ? filteredLoanList : AllLoans} />
                                        </Grid>
                                    </Grid>
                                </Grid>
                            </form>
...
export default connect(null, { loanList })(GradeAUser);
