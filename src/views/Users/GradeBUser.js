...
const GradeBUser = () => {
    ...
    const Defaulters = AllLoans?.filter((i) => i?.installmentsDate?.filter((item) => item < currentDate)?.length > i?.installments?.length);
    ...
    return (
        <Grid container spacing={gridSpacing}>
            <Grid item xs={12}>
                <Card>
                    <h3 className={classes.title}>Grade B Customers</h3>
                    <Formik
                        initialValues={{
                            type: '',
                            date: '',
